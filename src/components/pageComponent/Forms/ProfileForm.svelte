<script>
  ...
  let profil;
    let errorMessage;
    const ranks = { 0: "Basic User", 1: "Admin", 2: "Owner" };
  ...
  async function updateUser() {
    ...
    const success = await asteoService.updateUser(profil);
    if (!success) {
  ...
  <div class="md:grid md:grid-cols-3 md:gap-6">
    <div class="ml-4 md:col-span-1">
      <ImageContent
        ...
        {contentList}
        imgSource="/design/images/artworks/Waves.jpg"
        imgCaption={["Fig.1 - Unkown Artist - Waves, 1887."]}
      />
    </div>
          ...
          <div class="px-4 py-5 bg-white sm:p-6">
            {#if profil}
              <div class="grid grid-cols-6 gap-6">
                ...
                <img
                  src={profil.image}
                  class="col-span-2 row-span-4 border-stone-400 bg-white my-auto block border border-2 rounded-full"
                  alt=""
                />
