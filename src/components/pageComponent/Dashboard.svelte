...
let numberOfUsers, numberOfPosts, numberOfVisitors, numberOfLocations;

let inputAbsDigits, inputChart = [];
  ...
  onMount(async () => {
    ...
    const epochOccurences = paintings.reduce((acc, curr) => {
    ...
    const galleryOccurences = paintings.reduce((acc, curr) => {
      return acc[curr] ? ++acc[curr] : acc[curr] = 1, acc;
    }, {});
