<script>
  ...
  async function loadUserTable() {
    ...
    editMode = users.map(() => false);
    console.log(editMode);
  }

  async function updateUser(user, idx) {
    const success = await asteoService.updateUser(user);
    editMode[idx] = !editMode[idx];
    if (!success) {
  ...
  async function deleteUser(id) {
    const res = await asteoService.deleteUser(id);
    try {
      if (res.status === 204) {
        loadUserTable();
      }
          ...
          <tr class="">
            <td class="">
              <form
                ...
                class="py-2 grid grid-cols-8 border-b child:my-5 dark:bg-gray-800 dark:border-gray-700 odd:bg-white even:bg-gray-50 odd:dark:bg-gray-800 even:dark:bg-gray-700"
              >
                <img
                  src={user.image}
                  class="col-span-1 border-stone-400 bg-white my-auto h-16 col-span-1 row-span-2  block border border-2 rounded-full"
                  alt=""
                />
                ...
                <div class="col-span-1 flex items-center space-x-4">
                  {#if !editMode[idx]}
                    <button on:click={() => (editMode[idx] = !editMode[idx])} class="shadow-md bg-white rounded-lg hover:bg-slate-200">
                  ...
                  {:else}
                    <button
                      on:click={() => updateUser(user, idx)}
                      class="text-pastelBlackEerie font-mono py-2 px-2 border-r-2 border-pastelMediumPurple hover:bg-white hover:border-pastelBlackOlive transition duration-300"
