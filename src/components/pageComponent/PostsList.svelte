<script>
  ...
  async function createComment() {
    const success = await asteoService.createPost(galleryId, { rating, headline, comment });
    rating = 0;
    headline = "";
    comment = "";
    if (success) {
...
<div class="container mt-12 p-8 border-pastelBlackOlive border-t-2">
  <div class="flex flex-col">
    <div class="flex justify-between">
      ...
      <button
        on:click={() => (commentHidden = !commentHidden)}
        class="border border-4 bg-white border-pastelBlackOlive px-4 py-2 rounded-lg hover:bg-slate-200"
      >
    ...
    {#if ownProfil}
      <div class="bg-sky-200 shadow-lg w-1/2 border border-stone-400 border-2 mx-auto rounded-lg p-8 {commentHidden ? 'hidden' : ''}">
        <form on:submit|preventDefault={createComment}>
          <CommentCreate image={ownProfil.image} bind:rating bind:headline bind:comment />
          {#if errorMessage}
            <div class="">
              {errorMessage}
            </div>
      ...
      <div class="grid grid-cols-12 gap-4 p-4 shadow-lg">
        ...
        <div class="col-span-1 flex items-center ">
          <button
            ...
            class="mx-auto border border-2 bg-white border-pastelBlackOlive px-2 rounded-lg hover:bg-slate-200"
          >
