<script>
  ...
  let paintingList; let idList; let attrList; let imgList; let formHidden; let
  errorMessage;
  let title; let year; let price; let
  image;
  let epochsList = [];
    let selectedEpoch;
    let artistsList = [];
    let selectedArtist;
    let galleriesList;
    let selectedGallery; // selected attributes by Option
  ...
  async function updateGallery() {
    ...
    attrList = paintingList.map((ele) => [`Title: ${ele.title}`, `Year: ${ele.year}`]);
  ...
  async function createArtist() {
   ...
   title, year, price, image, epoch, artist, gallery,
  });
      if (success) {
...
<GalleryHorizontal bind:imgList bind:input={attrList} bind:refList={idList} bind:galleryList={paintingList} />
<Footer />
